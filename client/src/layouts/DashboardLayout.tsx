    ...
    <div className="min-h-screen bg-white md:grid md:grid-cols-[240px,1fr]">
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 h-20 border-b border-slate-200/50 bg-gradient-to-r from-white to-slate-50/50">
          ...
          <button onClick={() => setSidebarOpen(false)} className="w-8 h-8 bg-gradient-to-r from-slate-100 to-slate-200 rounded-lg flex items-center justify-center hover:from-slate-200 hover:to-slate-300 transition-all duration-300 shadow-sm hover:scale-110 border border-slate-300/50 md:hidden" aria-label="Close sidebar">
        ...
        <nav className="p-4 space-y-2">
          {navItems
            .filter((item) => {
              ...
              if (role === 'admin') return ['Dashboard','Question Bank','Create Questions','Approvals','Assignments','Analytics','User Management'].includes(item.label)
              if (role === 'editor') {
                return ['Dashboard', 'Question Bank', 'Create Questions', 'Approvals'].includes(item.label)
              }
              if (role === 'recruiter') {
                // Recruiters should not access Question Bank / AI generator
                return ['Dashboard', 'Assignments', 'Analytics'].includes(item.label)
              }
              // candidate - Dashboard for overview, Results for detailed analytics
              return ['Dashboard', 'Results'].includes(item.label)
            })
            .map((item) => {
