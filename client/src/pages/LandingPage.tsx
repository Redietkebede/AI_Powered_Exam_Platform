  ...
  return (
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-10">
          ...
          <div className="absolute top-80 right-80 w-12 h-12 bg-white rounded-full"></div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
             ...
             <div className="text-white">
               ...
               <p className="text-xl leading-relaxed text-white/90 mb-8 max-w-2xl">
                 Our AI‑Powered Exam Platform helps you assess skills with intelligent question generation, adaptive testing, and comprehensive analytics—fast, fair, and at scale.
               </p>
             ...
             <div className="grid gap-4">
                 ...
                 >
                   <div className="flex items-center gap-3">
                     <div className="h-9 w-9 rounded-md bg-[#ff7a59] text-white grid place-items-center text-sm font-bold">AI</div>
                     <div>
                                            <div className="text-white font-medium">Adaptive Engine</div>
                     <div className="text-white/70 text-sm">Questions adjust in real‑time for precision.</div>
                     </div>
                   </div>
                 </button>
                                <button
                   className="hover-zoom rounded-xl border border-white/15 bg-white/5 p-5 text-left shadow-lg backdrop-blur transition hover:bg-white/10 active:animate-zoom-pop animate-float-slow" style={{animationDelay:'100ms'}}
                   onClick={() => {}}
                   type="button"
                 >
                   <div className="flex items-center gap-3">
                     <div className="h-9 w-9 rounded-md bg-white/10 text-white grid place-items-center text-sm font-bold">∑</div>
                     <div>
                                            <div className="text-white font-medium">AI Question Generation</div>
                     <div className="text-white/70 text-sm">High‑quality items, on demand.</div>
                     </div>
                   </div>
                 </button>
                                <button
                   className="hover-zoom rounded-xl border border-white/15 bg-white/5 p-5 text-left shadow-lg backdrop-blur transition hover:bg-white/10 active:animate-zoom-pop animate-float-slow" style={{animationDelay:'200ms'}}
                   onClick={() => {}}
                   type="button"
                 >
                   <div className="flex items-center gap-3">
                     <div className="h-9 w-9 rounded-md bg-white/10 text-white grid place-items-center text-sm font-bold">📊</div>
                     <div>
                                            <div className="text-white font-medium">Insights & Analytics</div>
                     <div className="text-white/70 text-sm">See strengths, gaps, and trends instantly.</div>
                     </div>
                   </div>
                 </button>
             </div>
       ...
       <footer className="relative z-10 border-t border-white/15 bg-gradient-to-r from-slate-800 via-slate-700 to-[#ff7a59]">
         <div className="mx-auto max-w-7xl px-6 py-6 text-sm text-white/70">
           © {new Date().getFullYear()} AI‑Powered Exam Platform · Transforming Assessment Technology
         </div>
       </footer>
    </div>
  )
...
function RotatingText() {
  ...
  useEffect(() => {
    const id = setInterval(() => setIndex((v) => (v + 1) % phrases.length), 2500)
    return () => clearInterval(id)
  }, [])

  return (
    <span className="inline-block min-h-[1.5em] align-top hover-zoom">
      <span key={index} className="inline-block">{phrases[index]}</span>
    </span>
  )
}
