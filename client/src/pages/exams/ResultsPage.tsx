...
export default function ResultsPage() {
  ...
  // Custom CSS to override default select styling
  const customStyles = `
  ...
  const selectedAttempt: ExamAttempt | null = useMemo(() => 
    candidateAttempts.find(a => a.attemptId === selectedAttemptId) ?? candidateAttempts[0] ?? null, 
    [candidateAttempts, selectedAttemptId]
  )

  const subjects = useMemo(() => Array.from(new Set(questions.map(q => q.subject))).sort(), [questions])

  // Calculate overall candidate performance summary
  const candidateStats = useMemo(() => {
    const totalAttempts = candidateAttempts.length
    const totalQuestions = candidateAttempts.reduce((sum, a) => sum + a.items.length, 0)
    const correctAnswers = candidateAttempts.reduce((sum, a) => sum + a.items.filter(i => i.correct).length, 0)
    const avgScore = totalAttempts > 0 
      ? Math.round(candidateAttempts.reduce((sum, a) => sum + (a.items.filter(i => i.correct).length / a.items.length * 100), 0) / totalAttempts)
      : 0
  ...
  const subjectStats = useMemo(() => {
    if (!selectedAttempt) return [] as { subject: string; correct: number; total: number }[]
    ...
    return subjects.map(s => ({ subject: s, correct: map[s]?.correct ?? 0, total: map[s]?.total ?? 0 }))
  ...
  const difficultyStats = useMemo(() => {
    ...
    return order.map(d => {
      ...
      const acc = items.length ? Math.round((items.filter(i=>i.correct).length / items.length) * 100) : 0
      return { label: d, accuracy: acc }
    })
  }, [selectedAttempt])

  const timeSeries = useMemo(() => {
    if (!selectedAttempt) return { labels: [], scores: [] }
    const labels = selectedAttempt.items.map((_, idx) => `Q${idx+1}`)
    const accProgress = selectedAttempt.items.map(i => (i.correct ? 1 : 0))
    const cum = [] as number[]
    let sum = 0
    for (let i=0; i<accProgress.length; i++) { sum += accProgress[i]; cum.push(Math.round((sum/(i+1))*100)) }
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="text-center p-5 bg-gradient-to-br from-[#ff7a59]/5 to-[#ff7a59]/10 rounded-xl border border-[#ff7a59]/20 hover:shadow-md transition-all duration-200">
            <div className="text-3xl font-bold text-[#ff7a59] mb-2">{candidateStats.totalAttempts}</div>
          ...
          <div className="text-center p-5 bg-gradient-to-br from-green-500/5 to-green-500/10 rounded-xl border border-green-200 hover:shadow-md transition-all duration-200">
            <div className="text-3xl font-bold text-green-600 mb-2">{candidateStats.avgScore}%</div>
          ...
          <div className="text-center p-5 bg-gradient-to-br from-blue-500/5 to-blue-500/10 rounded-xl border border-blue-200 hover:shadow-md transition-all duration-200">
            <div className="text-3xl font-bold text-blue-600 mb-2">{candidateStats.correctAnswers}/{candidateStats.totalQuestions}</div>
          ...
          <div className="text-center p-5 bg-gradient-to-br from-purple-500/5 to-purple-500/10 rounded-xl border border-purple-200 hover:shadow-md transition-all duration-200">
            <div className="text-3xl font-bold text-purple-600 mb-2">{candidateStats.overallAccuracy}%</div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-3">Choose an attempt to view detailed results:</label>
          <select 
              ...
              <table className="w-full">
                ...
                <tbody>
                  {subjectStats.map((row, index) => (
                    <tr key={row.subject} className={`border-b border-gray-100 ${index % 2 === 0 ? 'bg-gray-50/50' : 'bg-white'}`}>
                      ...
                      <td className="p-4">
                        <span className={`font-bold text-lg ${row.total > 0 ? (row.correct / row.total >= 0.7 ? 'text-green-600' : row.correct / row.total >= 0.5 ? 'text-[#ff7a59]' : 'text-red-500') : 'text-gray-400'}`}>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {selectedAttempt?.items.map((it, idx) => (
                    <tr key={`${it.questionId}-${idx}`} className={`border-b border-gray-100 ${idx % 2 === 0 ? 'bg-gray-50/50' : 'bg-white'}`}>
                      <td className="p-4 font-medium text-[#ff7a59]">Q{idx+1}</td>
                      <td className="p-4 text-gray-700">{it.subject}</td>
                      <td className="p-4">
                        <span className="px-3 py-1.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700">{it.difficulty}</span>
                      </td>
                      <td className="p-4">
                        <span className="px-3 py-1.5 rounded-full text-xs font-medium bg-[#ff7a59]/10 text-[#ff7a59]">{it.type}</span>
                      </td>
                      <td className="p-4">
                        <span className={`px-3 py-1.5 rounded-full text-sm font-medium ${it.correct ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                          {it.correct ? 'Correct' : 'Incorrect'}
                        </span>
                      </td>
                      <td className="p-4 text-gray-700">{Math.round(it.timeSpentMs / 1000)}s</td>
                    </tr>
                  ))}
                  {(!selectedAttempt || selectedAttempt.items.length === 0) && (
                    <tr>
                      <td colSpan={6} className="py-8 text-center text-gray-500">No questions available.</td>
                    </tr>
                  )}
                </tbody>
              </table>
