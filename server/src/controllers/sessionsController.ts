...
export const submitSessionAnswers: RequestHandler = async (req, res) => {
  try {
    ...
    if (rawUserId == null) return res.status(401).json({ error: 'Unauthenticated' });
    ...
    answers.forEach((a, i) => {
      const o = i * 4;
      values.push(sessionId, Number(a.questionId), Number(a.selectedIndex), Number(a.timeTakenSeconds || 0));
